...
from dataset_fn import LLaDAPreparedChunkDataset
...
# model.set_activation_checkpointing(ActivationCheckpointingStrategy.full)
tokenizer = AutoTokenizer.from_pretrained("GSAI-ML/LLaDA-8B-Instruct")
hf_model = LLaDAModelLM(config=hf_configs, model=model)
print("Model test success")

dataset = LLaDAPreparedChunkDataset(
    "/data/sknigam/fineweb_10bt/pr5"
)
...
for step, batch in enumerate(dataloader, start=1):
    ...
    masked_targets = inp[mask].long()       # [N]

    ce = F.cross_entropy(masked_logits, masked_targets, reduction="mean")

    # diffusion weighting (stable)
    weight = (1.0 / t_vals.clamp(min=1e-3)).mean().detach()
    loss = ce * weight
